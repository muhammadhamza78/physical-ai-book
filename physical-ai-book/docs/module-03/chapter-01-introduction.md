...
1. **What are the three main components of the NVIDIA Isaac ecosystem?**
   - :::details AnswerIsaac Sim (simulation), Isaac ROS (GPU perception), Nav2 (navigation):::

2. **Why is GPU acceleration important for robotics?**
   - :::details AnswerGPUs process sensor data 10x faster than CPUs, enabling real-time perception at 30+ Hz needed for dynamic robot movement.:::
...
4. **What does cuVSLAM stand for and what does it do?**
   - :::details AnswerCUDA Visual SLAM - GPU-accelerated algorithm that tracks robot position and builds a map using camera and IMU data.:::

5. **How is Isaac Sim different from Gazebo?**
   - :::details 
AnswerIsaac Sim provides photorealistic rendering (for realistic vision), GPU-accelerated physics, built-in synthetic data generation, and tighter integration with NVIDIA's perception stack.
:::
